...
export class SceneRig {
  ...
  /**
   * Places that must stay free of scenery: the spawn point, and every boss
   * arena. Derived from the zone rather than hardcoded — a hardcoded clearing
   * silently stopped matching when the boss moved, and a tree ended up planted
   * straight through Old Scar.
   */
  private clearings(zone: ZoneDef): Array<{ x: number; z: number; r: number }> {
    const out = [{ x: zone.playerStart.x, z: zone.playerStart.z, r: 11 }];
