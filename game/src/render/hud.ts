...
const SKILL_ORDER = ['strike', 'rend', 'rally', 'bulwark', 'sunder'];
...
export class Hud {
      ...
      switch (ev.t) {
        case 'damage': {
          ...
          if (dealt || ev.targetId === playerId) {
            const skill = ev.skillId ? ` (${getSkill(ev.skillId).name})` : '';
            this.log(
              dealt
        ...
        case 'castInterrupted':
          if (ev.sourceId === playerId) {
            this.log(`${getSkill(ev.skillId).name} was interrupted.`, 'log-warn');
          }
          break;
        case 'error':
  ...
  private updateTargetFrame(): void {
    ...
    const rank = target.kind === 'mob' ? getMob(target.defId!).rank : 'normal';
    this.els.targetName.textContent = target.dead ? `${target.name} (dead)` : target.name;
    this.els.targetLevel.textContent = `Lv ${target.level}`;
    this.els.targetLevel.className = `frame-level rank-${rank}`;
  ...
  private updateCastBar(player: Entity): void {
    ...
    const skill = getSkill(player.cast.skillId);
    const progress = 1 - player.cast.remainingMs / skill.castMs;
    this.els.castBar.style.display = 'block';
    this.els.castFill.style.width = `${Math.max(0, Math.min(1, progress)) * 100}%`;
    this.els.castLabel.textContent = skill.name;
  }
  ...
  private updateNameplates(camera: THREE.Camera): void {
    ...
    for (const entity of this.world.entities.values()) {
      ...
      const tag = rank === 'boss' ? '◆ ' : rank === 'elite' ? '◈ ' : '';
      const name = plate.querySelector<HTMLElement>('.np-name')!;
      name.textContent = lootable
        ? `${entity.name} — press F to loot`
        : `${tag}${entity.name} (${entity.level})`;
...
const TEMPLATE = `
  ...
  <div id="help">
    ...
    <b>Click</b> / <b>Tab</b> target &nbsp; <b>1–5</b> skills &nbsp; <b>T</b> auto-attack<br />
